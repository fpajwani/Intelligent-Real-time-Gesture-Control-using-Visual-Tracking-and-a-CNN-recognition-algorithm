...
output = ["Empty","Five","Good"]

jsonarray = {}

#%%
def update(plot):
    global jsonarray,prob_array
    h = 450
    y = 30
    w = 45
    temp=np.str()
    ...
    for items in jsonarray:
        ...
        # for size in range(0,nb_classes):
        #     cv2.putText(plot, np.str(temp),(w+30,y+5),font,0.7,(0,0,0))
        #     a=prob_array[0][size]
        #     cv2.putText(plot, np.str(a),(w+30,y+5),font,0.7,(255,255,255))
        #     temp=a
            
        cv2.putText(plot,items,(0,y+5), font , 0.7,(0,255,0),2,1)
        # cv2.putText(plot,prob_array,(0,y+10),font,0.7,(0,255,0),2,1)
        y = y + w + 30
        print("Accuracy:",prob_array)
...
# CNN model
    ...
    if not bTraining :
        ...
        print("The model is running on the selected weights...")
...
def guessGesture(model, img):
    global output, get_output, jsonarray,prob_array
